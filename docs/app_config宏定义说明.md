...
当前默认路径为：`main()` 在 `ENABLE_CONTEST_TASKS=1` 下进入 `run_task_dispatcher()`。正式任务入口是 `run_task1_ab()`、`run_task2_abcd()`、`run_task3_acbda()`、`run_task4_four_laps()`；任务 5/6/7/10/11 属于调试或专项验证入口。

## 总体情况

`app_config.h` 当前包含约 355 个配置宏。绝大多数仍有文本引用，但文本引用不等同于实际生效。核对后有 23 个宏当前完全无源码引用，另有若干宏只在 `app_config.h` 的派生表达式中使用，还有一批参数主要被当前任务入口已确认不可达的旧函数引用（2026-06-08 经 codegraph 验证为 0 callers）。
...
以下宏当前没有被应用代码、BSP 或 Board 引用，也不参与 `app_config.h` 内仍生效的派生链。它们不会直接影响当前固件行为。
...
## 被旧代码引用但当前任务确认不生效的参数（2026-06-08 codegraph 验证）

以下参数不是“完全无引用”。它们在源码中仍能搜到，但**全部出现在已确认不可达的旧函数中**（经 codegraph 验证为 0 callers）。整理时应优先删除旧函数，再删除专属参数。
