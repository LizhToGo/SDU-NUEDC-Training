...
本文档对应 `Board/board.h` 和 `Board/board.c`。这个模块不是具体外设驱动，而是工程里的通用工具层，主要提供串口打印和延时函数。

## 1. 文件作用

| 文件 | 作用 |
|---|---|
| `Board/board.h` | 对外声明打印函数和延时函数 |
| `Board/board.c` | 实现 UART0 发送、`printf` 重定向、`lc_printf()` 和延时函数 |
...
`SYSCFG_DL_init()` 会初始化 `UART_0_INST`，之后 `lc_printf()` 和 `printf()` 才能正常通过串口输出。
...
2. `lc_printf()` 是阻塞式发送，频繁打印会影响主循环速度。
