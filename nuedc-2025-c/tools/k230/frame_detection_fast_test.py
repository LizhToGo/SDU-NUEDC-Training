"""Balanced nested-frame detection test for Yahboom K230 + GC2093.

This variant uses one 512x288 RGB565 image for both detection and preview.
Every detection searches the complete image: this is more responsive to a
large camera movement than an unreliable tracking ROI, while retaining much
more edge detail than the previous 320x180 grayscale experiment.
...
# Keep image dimensions and display offsets aligned to 16 pixels.  The K230
# RGB565/display path corrupts rows for the earlier 480x270 image because its
# 270-line buffer and y=105 placement do not satisfy the backend alignment.
PREVIEW_WIDTH = 512
PREVIEW_HEIGHT = 288
...
# K230 find_rects was unable to return a valid result from the tracking ROI
# reliably (0/89 in the 320x180 run and mostly fallbacks at 640x360).  A single
# full 512x288 scan is faster and safer than ROI + full-frame double scanning.
...
try:
    ...
    print("")
    print("Balanced nested-frame detector ready.")
    print("CanMV IDE/LCD preview is running.")
    print(
        "Detection/preview channel:",
        "%dx%d RGB565" % (PREVIEW_WIDTH, PREVIEW_HEIGHT),
    )
    print("RECT_THRESHOLD:", RECT_THRESHOLD)
    print("Tracking ROI: disabled (single full-frame scan)")
